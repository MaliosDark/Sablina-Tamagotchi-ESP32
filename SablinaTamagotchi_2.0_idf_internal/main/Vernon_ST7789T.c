...
static esp_err_t panel_st7789t_init(esp_lcd_panel_t *panel)
{
    ...
    // printf("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\r\n");
    /* Exact Waveshare ESP32-S3-LCD-1.47 init sequence from official Arduino demo */
    esp_lcd_panel_io_tx_param(io, 0x11, NULL, 0);  /* SLPOUT */
    vTaskDelay(pdMS_TO_TICKS(120));

    esp_lcd_panel_io_tx_param(io, 0x36, (uint8_t []){0x00}, 1);  /* MADCTL – will be overwritten by swap_xy/mirror */
    esp_lcd_panel_io_tx_param(io, 0x3A, (uint8_t []){0x05}, 1);  /* COLMOD: 16bpp */
    ...
    esp_lcd_panel_io_tx_param(io, 0xBB, (uint8_t []){0x35}, 1);
    ...
    esp_lcd_panel_io_tx_param(io, 0xD6, (uint8_t []){0xA1}, 1);  /* missing in Vernon, present in Waveshare demo */

    /* Gamma – exact Waveshare values */
    esp_lcd_panel_io_tx_param(io, 0xE0, (uint8_t []){0xF0,0x00,0x04,0x04,0x04,0x05,0x29,0x33,0x3E,0x38,0x12,0x12,0x28,0x30}, 14);
    esp_lcd_panel_io_tx_param(io, 0xE1, (uint8_t []){0xF0,0x07,0x0A,0x0D,0x0B,0x07,0x28,0x33,0x3E,0x36,0x14,0x14,0x29,0x32}, 14);

    esp_lcd_panel_io_tx_param(io, 0x21, NULL, 0);  /* INVON */
    esp_lcd_panel_io_tx_param(io, 0x11, NULL, 0);  /* SLPOUT again (Waveshare demo does this) */
    vTaskDelay(pdMS_TO_TICKS(120));
