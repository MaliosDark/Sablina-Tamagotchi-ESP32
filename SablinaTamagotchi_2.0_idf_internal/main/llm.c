...
void malloc_run_state(RunState *s, Config *p)
{
    ...
    ESP_LOGI(TAG, "run_state alloc: dim=%d hidden=%d n_layers=%d seq=%d kv_dim=%d vocab=%d",
             p->dim, p->hidden_dim, p->n_layers, p->seq_len, kv_dim, p->vocab_size);
    ESP_LOGI(TAG, "SPIRAM free before alloc: %u", (unsigned)heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
    s->x = heap_caps_calloc(p->dim, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    ESP_LOGI(TAG, "x=%p", s->x);
    s->xb = heap_caps_calloc(p->dim, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    ESP_LOGI(TAG, "xb=%p", s->xb);
    s->xb2 = heap_caps_calloc(p->dim, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    s->hb = heap_caps_calloc(p->hidden_dim, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    s->hb2 = heap_caps_calloc(p->hidden_dim, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    s->q = heap_caps_calloc(p->dim, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    ESP_LOGI(TAG, "key_cache size=%u", (unsigned)(p->n_layers * p->seq_len * kv_dim * sizeof(v4sf)));
    s->key_cache = heap_caps_calloc(p->n_layers * p->seq_len * kv_dim, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    ESP_LOGI(TAG, "key_cache=%p", s->key_cache);
    s->value_cache = heap_caps_calloc(p->n_layers * p->seq_len * kv_dim, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    ESP_LOGI(TAG, "value_cache=%p", s->value_cache);
    s->att = heap_caps_calloc(p->n_heads * p->seq_len, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    s->logits = heap_caps_calloc(p->vocab_size, sizeof(v4sf), MALLOC_CAP_SPIRAM);
    ESP_LOGI(TAG, "SPIRAM free after alloc: %u", (unsigned)heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
                     ...
                     int *fd, v4sf **data, size_t *file_size)
{
    ...
    // read in the config header — detect v2 format (magic 0x616B3432 = "ak42")
    int weights_offset = (int)sizeof(Config); // v1: weights immediately after Config
    uint32_t magic_num;
    if (fread(&magic_num, sizeof(uint32_t), 1, file) != 1) { exit(EXIT_FAILURE); }
    if (magic_num == 0x616B3432u) {
        // v2 format: 4-byte magic + 4-byte version + 28-byte Config + padding → 256-byte header
        int version;
        if (fread(&version, sizeof(int), 1, file) != 1) { exit(EXIT_FAILURE); }
        if (fread(config, sizeof(Config), 1, file) != 1) { exit(EXIT_FAILURE); }
        weights_offset = 256; // weights start at byte 256
        ESP_LOGI(TAG, "Model format: v2 (version=%d)", version);
    } else {
        // v1 format: magic_num is actually the first field (dim) of Config
        fseek(file, 0, SEEK_SET);
        if (fread(config, sizeof(Config), 1, file) != 1) { exit(EXIT_FAILURE); }
        ESP_LOGI(TAG, "Model format: v1");
    }
    // negative vocab size is hacky way of signaling unshared weights. bit yikes.
    int shared_weights = config->vocab_size > 0 ? 1 : 0;
    config->vocab_size = abs(config->vocab_size);
    ESP_LOGI(TAG, "Config: dim=%d hidden=%d layers=%d heads=%d kv_heads=%d vocab=%d seq=%d",
    ...
    *data = heap_caps_malloc(*file_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    ...
    ESP_LOGI(TAG, "Free ram available: %lu", esp_get_free_heap_size());
    v4sf *weights_ptr = (v4sf *)((char *)*data + weights_offset);
...
void generate_with_callbacks(Transformer *transformer,
                             Tokenizer *tokenizer,
                             Sampler *sampler,
                             char *prompt,
                             int steps,
                             generated_complete_cb cb_done,
                             generated_token_cb cb_token)
{
    char *empty_prompt = "";
    if (prompt == NULL)
    {
...
{
    generate_with_callbacks(transformer, tokenizer, sampler, prompt, steps, cb_done, NULL);
}
