...
// ── LLM / storage paths ───────────────────────────────────────────────────
#define CHECKPOINT_PATH   "/data/stories260K.bin"
#define TOKENIZER_PATH    "/data/tok512.bin"

// ── Agent timing ─────────────────────────────────────────────────────────
#define THINK_STEPS           32     // LLM tokens per thought (~1-2 s on S3)
...
static void agent_task(void *arg)
{
    ...
    while (1) {
        ...
        generate_with_callbacks(&transformer, &tokenizer, &sampler,
                                prompt, THINK_STEPS, complete_cb, token_cb);
